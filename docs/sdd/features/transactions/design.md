...
- **Context note:** The compact period context includes explanatory copy that rows use the transaction date and totals use installment competence. This is explanatory copy, not a second viewing mode.
- **Expense drill-down:** The expense summary card links to `/expenses/breakdown` with the active temporal state serialized in URL parameters.
